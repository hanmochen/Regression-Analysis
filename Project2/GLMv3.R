...
# First, we model3 the claims frequency. 
# The first part of this performs a GLM analysis, with glmdata2 as the data source model3ling NoOfClaims, by the Duration. It looks at three variables: weight_group, Climate, and ActivityCode.
##### This is where you can modify the model3 by adding or removing variables 
...
##### This part needs to be modified if you change which variables are included in the model3, but not if you change the groups inside a variable
...
# We then do the same thing again, now model3ling severity instead of claim frequency.
...
# Then we do the same thing as we did when model3ling claims frequency, but we look at average claim;
# A GLM analysis is run, the coefficients stored, and saved in a new column, named rels.severity, glmdata3
##### You need to modify this part of the code in the same way as you did for the GLM model3 for frequency.  Add or remove variables
...
##### Congratulations! You now have a model3 for the risk!
...
##### If you have changed what variables are included in your model3, add, remove, or modify sections of this code to plot them. 
...
write.xlsx(glmdata3, "factors3.xlsx")
