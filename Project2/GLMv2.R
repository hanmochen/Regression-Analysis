...
glmdata$weight_group <- cut(glmdata$Weight, 
                            breaks = c(-Inf, 454, 1000, 2330, 3830, Inf), 
                            labels = c("01_<454kg", "02_454-999kg", "03_1000-2329kg", "04_2330-3829kg", "05_>3830kg"), 
                            right = FALSE)

glmdata$age_group <- cut(glmdata$VehicleAge, 
                         breaks = c(-Inf, 3, 6, 11,20, Inf), 
                         labels = c("01_<3years", "02_3-5years", "03_6-10years", "04_11-19years","05_>20years"), 
                         right = FALSE)
...
# First, we model2 the claims frequency. 
# The first part of this performs a GLM analysis, with glmdata2 as the data source model2ling NoOfClaims, by the Duration. It looks at three variables: weight_group, Climate, and ActivityCode.
##### This is where you can modify the model2 by adding or removing variables 
...
##### This part needs to be modified if you change which variables are included in the model2, but not if you change the groups inside a variable
...
# We then do the same thing again, now model2ling severity instead of claim frequency.
...
# Then we do the same thing as we did when model2ling claims frequency, but we look at average claim;
# A GLM analysis is run, the coefficients stored, and saved in a new column, named rels.severity, glmdata3
##### You need to modify this part of the code in the same way as you did for the GLM model2 for frequency.  Add or remove variables
...
##### Congratulations! You now have a model2 for the risk!
...
##### If you have changed what variables are included in your model2, add, remove, or modify sections of this code to plot them. 
...
write.xlsx(glmdata3, "factors2.xlsx")
